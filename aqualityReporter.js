...
class AqualityReporter {
    constructor(config) {
        this.aqalityAPI = new AqalityAPI(config.token, config.project_id, config.apiURL)
        this.config = config;
        this.testrun = {
    ...
    jasmineStarted() {
        this.suite = this.aqalityAPI.createOrUpdateSuite({ name: this.config.suiteName, project_id: this.config.project_id });
